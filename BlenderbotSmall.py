...
BLENDERBOT_SMALL_GENERATION_EXAMPLE = r"""
    Conversation example::
        ...
        >>> print("Human: ", REPLY)
        >>> NEXT_UTTERANCE = (
        transformers. "My friends are cool but they eat too many carbs.</s> "
        transformers. "<s>what kind of carbs do they eat? i don't know much about carbs.</s> "
        transformers. "<s>I'm not sure."
        transformers. )
...
class TFBlenderbotSmallForConditionalGeneration(TFBlenderbotSmallPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.wte = TFSharedEmbeddings(
            config.vocab_size, config.hidden_size, initializer_range=config.init_std, name="wte"
        )
        self.final_logits_bias = self.add_weight(name="bias", shape=(1, config.vocab_size), initializer="zero", trainable=False)
    ...
    def call(
        ...
        **kwargs,
        ...
        lm_logits = self.wte(outputs[0], mode="linear") + self.final_logits_bias
...
if __name__ == '__main__':
    ...
    inputs = tokenizer(["你好啊，我叫唐小书。", "我也是中国的，我来自辽宁省大连市，你来自哪个省呢？唐恩达。"], return_tensors="tf", padding=True,
                       truncation=True)
    labels = tokenizer(["唐小书，你好，我叫唐恩达，是来自中国，你也哪里人呢？", "我也是来自辽宁省大连市的，很高兴认识你，唐小书。"], return_tensors="tf", padding=True,
                       truncation=True)

    model_config = BlenderbotSmallConfig.from_json_file("model_file/config_small.json")
    model = TFBlenderbotSmallForConditionalGeneration(config=model_config)
    model_inp = {
        "input_ids": inputs["input_ids"],
        "attention_mask": inputs["attention_mask"],
